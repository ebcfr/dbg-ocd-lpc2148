...
// Low level input/ouput functions
...
//	char d;
	...
	i = RxBuff.wi;
	cnt = RxBuff.ct;
//	while (uart_getchar(uart, &d)) {	/* Get all data in the Rx FIFO */
...
static char term_in(void)
{
	...
	uart->IER_DLM = 0;			/* Disable interrupts */
	i = RxBuff.ri;
	d = RxBuff.buff[i++];	/* Get a byte from Rx buffer */
	RxBuff.ri = i % FIFO_SIZE;
	RxBuff.ct--;
	uart->IER_DLM = 0x01;		/* Reenable interrupt */
 ...
 ***************************************************************************/
// init function
void term_init(UART *u, unsigned rows, unsigned cols)
{
	uart=u;
...
int readline (char *prompt_str, char *buf, unsigned maxlength) 
{
	int c;
